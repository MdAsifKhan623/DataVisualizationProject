#### BRFSS 2019 Data: Percentage Stacked Line Graph
...
### Percentage Stacked Line Graph:

brfss_new %>%
  group_by(sex,Month_Year) %>%
  summarise(cnt = n(),
            `.groups` = "drop")%>%
  as.data.frame() %>%
  #arrange(-cnt) %>%
  plot_ly(x = ~ Month_Year, 
          ...
          groupnorm = 'percent', fillcolor = ~ sex) %>%
  layout(title = "Distribution of survey respondents by Gender and Month-Year",
